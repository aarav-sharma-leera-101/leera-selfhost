...
BUNDLE_VERSION="${LEERA_BUNDLE_VERSION:-0.4.6}"

# The MinIO client, for copying bundled object storage in and out of a backup.
# MinIO's own registry: MinIO withdrew its images from Docker Hub in September
# 2026, and every install still pointing there lost its backups and updates
# at once. Pinned to the build Docker Hub's `latest` was at the time, which
# is the one those installs have. Keep in step with docker-compose.yml.
...
do_upgrade() {
  ...
  # Only from here does anything on this machine change: the stack files,
  # then the version they run.
  if [ "$REFRESH_BUNDLE" = "1" ]; then
    apply_bundle
  fi
