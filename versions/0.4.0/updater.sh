...
run_update() {
  ...
  local skip_backup digest_api digest_web digest_installer
  ...
  digest_installer="$(jq -r '.digests.installer // ""' "$CURRENT_FILE")"
  ...
  # installing something else. LEERA_INSTALLER_SHA256 is checked before the
  # installer hands the update over to the target release's copy of itself.
  ...
  local env_args=("LEERA_HOME=$INSTALL_DIR"
                  ...
                  "LEERA_INSTALLER_SHA256=$digest_installer")
  local project
