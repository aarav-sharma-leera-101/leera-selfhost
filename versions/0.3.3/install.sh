...
BUNDLE_VERSION="${LEERA_BUNDLE_VERSION:-0.3.3}"
...
apply_bundle() {
  ...
  if [ -f "$BUNDLE_STAGING/install.sh" ] && ! same_file install.sh "$BUNDLE_STAGING/install.sh"; then
    cp "$BUNDLE_STAGING/install.sh" install.sh.new
...
do_upgrade() {
  ...
  if [ "$REFRESH_BUNDLE" = "1" ]; then
    stage_bundle "$(resolve_bundle_version "$target_tag")"
  fi
  step_done preflight
